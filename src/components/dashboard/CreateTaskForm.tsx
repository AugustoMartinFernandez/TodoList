    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pb-3 pt-1 bg-slate-50/50 border-t border-slate-100">
        <div className="flex items-center gap-2">
          <button type="button" onClick={cyclePriority} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all border ${newTaskPriority === 'normal' ? 'border-transparent text-slate-500 hover:bg-slate-200' : priorityColors[newTaskPriority]}`}>
            <Flag className="w-3.5 h-3.5" /><span className="capitalize">{newTaskPriority}</span>
          </button>
          
          <div className="relative flex items-center">
            <input type="date" min={todayStr} value={newTaskDate} onChange={(e) => setNewTaskDate(e.target.value)} onClick={(e) => { try { if ('showPicker' in HTMLInputElement.prototype) e.currentTarget.showPicker(); } catch (err) {} }} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all border ${newTaskDate ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'border-transparent text-slate-500 hover:bg-slate-200'}`}>
            ...
            {newTaskDate && <button type="button" onClick={() => setNewTaskDate("")} className="absolute right-1 z-20 p-1 text-indigo-400 hover:text-indigo-600 bg-indigo-50 rounded-full"><X className="w-3 h-3" /></button>}
          </div>
        </div>
